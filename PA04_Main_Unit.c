...
int main( int argc, char **argv )
   {
    ...
    ASSERT( rooms != NULL ) << "Room array creation failed!";

    courses = createCourseArray( MAX_NUM_COURSES );
    ASSERT( courses != NULL ) << "Course array creation failed!";

    ASSERT( roomLevel >= 0 ) << "Invalid room level!";
 
    if( uploadData( inputFilename, courses, rooms ) ) 
       {

        ASSERT( uploadData( inputFilename, courses, rooms ) == true ) << "Data upload failed!"; 
    ...
    // end program
       ...
       ASSERT( rooms == NULL ) << "Rooms array free process failed!";

       courses = clearCourseArray( courses );
       ASSERT( courses == NULL ) << "Course array free process  failed!";
...
bool fitClassRooms( CourseArrayType *courses, 
                         RoomArrayType *rooms, int roomLevel, bool verboseFlag )
   {
    ...
    ASSERT( localRoom.roomNumber == rooms->array[roomLevel].roomNumber ) << "Room Number doesn't match after deepcopy";
    ASSERT( localRoom.roomCapacity == rooms->array[roomLevel].roomCapacity ) << "Room Capacity doesn't match after deepcopy";
    ASSERT( localRoom.associatedIndex == rooms->array[roomLevel].associatedIndex ) << "Room Index doesn't match after deepcopy";
    ASSERT( strcmp(localRoom.buildingName, rooms->array[roomLevel].buildingName) == 0 ) << "Room Name doesn't match after deepcopy";
    ...
    for( courseIndex = 0; courseIndex < courses->size; courseIndex++ )
       {
        ...
        ASSERT( localCourse.size == courses->array[courseIndex].size ) << "Course Size doesn't match after deepcopy";
        ASSERT( localCourse.available == courses->array[courseIndex].available ) << "Course Availibility doesn't match after deepcopy";
        ASSERT( strcmp( localCourse.courseName, courses->array[courseIndex].courseName ) == 0 ) << "Course Name doesn't match after deepcopy";
        ...
        if( localCourse.available && 
               localCourse.size <= localRoom.roomCapacity )
           {
            ...
            ASSERT( localCourse.available == true ) << "Course not available after passing if() that says it is";
            ASSERT( localCourse.size <= localRoom.roomCapacity ) << "Course not available after passing if() that says it is";
